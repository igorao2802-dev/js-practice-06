...
🔗 [Посмотреть демо](https://igorao2802-dev.github.io/js-practice-06/) *(опционально)*  
...
![Результат удаления одной карточки](screenshots/delete-card.png)  
...
|---------|---------|--------|
| БАЗА | Задание 1 — Динамический список (создание/удаление) | ✅ Выполнено |
| JUNIOR | Задание 2 — Управление стилями и фильтрация | ✅ Выполнено |
| PRO | Задание 3 — Массовая вставка и работа с шаблонами | ✅ Выполнено |

### Общие требования
- [x] Проект запускается через Live Server, нет ошибок в Console  
- [x] Все обработчики событий — через `addEventListener` в `script.js`  
- [x] Пользовательский текст вставляется только через `textContent` (защита от XSS)  
- [x] Валидация: название не может быть короче 3 символов  
- [x] При удалении всех карточек — подтверждение через `confirm()`  
- [x] Комментарии к каждому `createElement`, `append`, `remove`, `classList`  
- [x] Использовать `querySelector` / `querySelectorAll` для поиска элементов  
- [x] Репозиторий оформлен: структура, README, папка screenshots  
- [x] Готов объяснить код устно на защите  

---

## 🎯 Уровни сложности

| Уровень | Задания | Для кого |
|---------|---------|----------|
| **БАЗА** | 1 | Для тех, кому тяжело (минимальные требования) |
| **JUNIOR** | 1-2 | Стандартный уровень (рекомендуется всем) ✅ |
| **PRO** | 1-3 | Для сильных студентов (дополнительные баллы) |

> 💡 **Совет для защиты**: Не заучивайте ответы — понимайте суть. Если спросят «а почему не `innerHTML`?», отвечайте: «`textContent` безопаснее — он экранирует HTML-теги и защищает от XSS-атак».
